...
export default function IncomeTaxPage() {
  ...
  const handleCalculate = () => {
    ...
    const totalDeductions = stdDed + advDed;
  ...
  const disabledCalc = !income || !stateAbbr;
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-sm text-center text-gray-600 dark:text-gray-400">
        Uses official 2025 brackets &amp; standard deductions, plus approximate state rates.
      </p>
      ...
      <div className="mt-4">
        <button
          ...
          className="flex items-center gap-1 text-sm text-indigo-600 dark:text-indigo-400
            hover:underline"
        >
          <span>{showDeductions ? "Hide" : "Show"} Advanced</span>
          <span className="text-xs">{showDeductions ? "▲" : "▼"}</span>
        </button>
      </div>
      {showDeductions && (
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 text-sm text-gray-700 dark:text-gray-200">
          {/* 401k */}
      ...
      {results && (
        <div className="mt-8">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
            * 2025 brackets & standard deductions. State rates approx. 
            If advanced inputs used, they reduce taxable. 
          </p>
